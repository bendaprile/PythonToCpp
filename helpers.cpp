...
vector < vector <float> > blur(vector < vector < float > > grid, float blurring) {
    ...
    vector<float> vector1 = {corner_prob, adjacent_prob, corner_prob};
    vector<float> vector2 = {adjacent_prob, center_prob, corner_prob};
    vector<float> vector3 = {corner_prob, adjacent_prob, corner_prob};
    vector < vector < float > > window;
    
    window.push_back(vector1);
    window.push_back(vector2);
    window.push_back(vector3);
    
    vector < vector < float > > newVector(3, vector<float>(3, 0));
        ...
        for(int j=0; j<width; j++) {
            ...
            for(int dx=-1; dx<2; dx++) {
                for(int dy=-1; dy<2; dy++) {
                    ...
                    new_i = abs((i + dy) % height);
                    new_j = abs((j + dx) % width);
                    newVector[new_i][new_j] = mult * grid_val;
                }
            }
        }
    }
